...
export default function App() {
  ...
  function addRunningHubWorkflow() {
    setRunningHubWorkflows((current) => [...current, { id: crypto.randomUUID(), displayId: nextPresetDisplayId("RH", current), workflowId: "", instanceType: "", bindings: [emptyRunningHubBinding] }]);
  }
        ...
        <section className="connections-page">
          ...
          <div className="connection-card runninghub-card">
            ...
            <div className="preset-card-list">{runningHubWorkflows.map((workflow) => {
              ...
              return <section className="preset-card" key={workflow.id}><div className="preset-card-header"><h3>{workflow.displayId}</h3><button onClick={() => setRunningHubWorkflows((current) => current.filter((item) => item.id !== workflow.id))} type="button">−</button></div><div className="runninghub-workflow-settings"><label><span>Workflow ID</span><input className="secret-input" onChange={(event) => updateRunningHubWorkflow(workflow.id, { workflowId: event.target.value })} value={workflow.workflowId} /></label><label><span>Использовать</span><select aria-label={`Режим RunningHub ${workflow.displayId}`} className="runninghub-instance-type-select" onChange={(event) => updateRunningHubWorkflow(workflow.id, { instanceType: event.target.value as RunningHubInstanceType })} value={workflow.instanceType}><option value="">Не выбрано</option><option value="standard">Standard</option><option value="plus">Plus</option></select></label></div><div className="runninghub-bindings"><div className="runninghub-bindings-title">Workflow bindings</div>{editableBindings.map((binding, index) => <div className="runninghub-binding-row" key={`${workflow.id}-${index}`}><label><span>Node ID</span><input className="secret-input" onChange={(event) => updateRunningHubWorkflow(workflow.id, { bindings: editableBindings.map((item, itemIndex) => itemIndex === index ? { ...item, nodeId: event.target.value } : item) })} value={binding.nodeId} /></label><label><span>Field</span><input className="secret-input" onChange={(event) => updateRunningHubWorkflow(workflow.id, { bindings: editableBindings.map((item, itemIndex) => itemIndex === index ? { ...item, fieldName: event.target.value } : item) })} value={binding.fieldName} /></label><label><span>Studio ID</span><select onChange={(event) => updateRunningHubWorkflow(workflow.id, { bindings: editableBindings.map((item, itemIndex) => itemIndex === index ? { ...item, studioId: event.target.value as RunningHubBinding["studioId"] } : item) })} value={binding.studioId}>{studioIds.map((studioId) => <option key={studioId} value={studioId}>{getStudioIdLabel(studioId)}</option>)}</select></label><button className="binding-icon-button" onClick={() => updateRunningHubWorkflow(workflow.id, { bindings: [...editableBindings, emptyRunningHubBinding] })} type="button">+</button><button className="binding-icon-button" onClick={() => updateRunningHubWorkflow(workflow.id, { bindings: editableBindings.length === 1 ? [emptyRunningHubBinding] : editableBindings.filter((_, itemIndex) => itemIndex !== index) })} type="button">−</button></div>)}</div></section>;
            })}</div>

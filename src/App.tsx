        ...
        <section className="connections-page">
          ...
          <div className="connection-card ollama-card">
            <div className="ollama-settings">
              <div className="preset-card-header"><h2>Ollama</h2><button onClick={addOllamaPreset} type="button">＋ Добавить Ollama</button></div>
          ...
          <div className="connection-card runninghub-card">
            <div className="preset-card-header">
              ...
              </div><button onClick={addRunningHubWorkflow} type="button">＋ Добавить workflow</button></div>
            <KeyActions disabled={isSavingKey} onClear={() => void handleClearKey("runningHubApiKey")} onEdit={() => handleEditKey("runningHubApiKey")} />
            <div className="preset-card-list">{runningHubWorkflows.map((workflow) => <section className="preset-card" key={workflow.id}><div className="preset-card-header"><h3>{workflow.displayId}</h3><button onClick={() => setRunningHubWorkflows((current) => current.filter((item) => item.id !== workflow.id))} type="button">−</button></div><label><span>Workflow ID</span><input className="secret-input" onChange={(event) => updateRunningHubWorkflow(workflow.id, { workflowId: event.target.value })} value={workflow.workflowId} /></label><div className="runninghub-bindings"><div className="runninghub-bindings-title">Workflow bindings</div>{workflow.bindings.map((binding, index) => <div className="runninghub-binding-row" key={`${workflow.id}-${index}`}><label><span>Node ID</span><input className="secret-input" onChange={(event) => updateRunningHubWorkflow(workflow.id, { bindings: workflow.bindings.map((item, itemIndex) => itemIndex === index ? { ...item, nodeId: event.target.value } : item) })} value={binding.nodeId} /></label><label><span>Field</span><input className="secret-input" onChange={(event) => updateRunningHubWorkflow(workflow.id, { bindings: workflow.bindings.map((item, itemIndex) => itemIndex === index ? { ...item, fieldName: event.target.value } : item) })} value={binding.fieldName} /></label><label><span>Studio ID</span><select onChange={(event) => updateRunningHubWorkflow(workflow.id, { bindings: workflow.bindings.map((item, itemIndex) => itemIndex === index ? { ...item, studioId: event.target.value as RunningHubBinding["studioId"] } : item) })} value={binding.studioId}>{studioIds.map((studioId) => <option key={studioId} value={studioId}>{getStudioIdLabel(studioId)}</option>)}</select></label><button className="binding-icon-button" onClick={() => updateRunningHubWorkflow(workflow.id, { bindings: [...workflow.bindings, emptyRunningHubBinding] })} type="button">+</button><button className="binding-icon-button" onClick={() => updateRunningHubWorkflow(workflow.id, { bindings: workflow.bindings.length === 1 ? [emptyRunningHubBinding] : workflow.bindings.filter((_, itemIndex) => itemIndex !== index) })} type="button">−</button></div>)}</div></section>)}</div>
          </div>
          <button
